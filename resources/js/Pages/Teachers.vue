...
// Open modal in edit mode and populate form with user data
const editUser = (user) => {
    mode.value = 'edit';
...
const submitForm = () => {
    if (mode.value === 'create') {
        ...
        form.put(route('teachers.update', selectedUser.value.id), {
            onSuccess: () => closeModal(),
        });
    }
};
...
watch(
    search,
    debounce((q) => router.get(route('teachers.index'), { search: q }, { preserveState: true }), 500)
);
                                ...
                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                    ...
                                    <form @submit.prevent="submitForm" class="p-4 md:p-7">
                                        ...
                                        <button type="submit"
                                            ...
                                            {{ mode === 'create' ? 'Add New Teacher' : 'Update Teacher' }}
                                        </button>
                                    </form>
                                </div>
                            </div>
                            ...
                            <template v-if="users.data.length > 0">
                                <tr v-for="(user, index) in users.data" :key="user.id"
                                    ...
                                    <td class="px-4 py-3 flex items-center justify-end gap-2">
                                        ...
                                        <button type="button" @click="destroyRow(user.id)" data-modal-target="popup-modal"
